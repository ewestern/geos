...
createWktReader = withGeos' $ \h -> do
    ...
    traverse (fmap WktReader . newForeignPtrEnv I.geos_WKTReaderDestroy h) eitherPtr
...
createWktWriter = withGeos' $ \h -> do
  ...
  traverse (fmap WktWriter . newForeignPtrEnv I.geos_WKTWriterDestroy h) eitherPtr
...
writeWkt (WktWriter w) g = withGeos $ \h ->  do
  wkt <- withForeignPtr w $ \wp ->
          withGeometry g $ \gp -> I.geos_WKTWriterWrite h wp gp
  BC.packCString wkt
